...
- `max-lab/CLAUDE.md` — **CATalyst Max build + test log.** The living context for the interview → synthesis → roadmap work: architecture (the `/interview` app + `/api/interview-chat` proxy, Gemini 2.5 Flash + Groq fallback), the 4 inline prompts + structured-profile schema, the **LOCKED REQUIREMENTS (never regress)** distilled from every founder test, the full feedback/iteration log, validation status, and the sim-harness validation discipline. **Read before touching any Max interview/roadmap prompt.** Related: `max-lab/INTERVIEW_PLAN.md`, `max-lab/ROADMAP_PLAN.md`, `max-lab/interview_sim.py` (test harness).
- `interview.html` (root) + `api/interview-chat.js` — the live Max validation app (`/interview`) and its secure key-holding proxy. Keys are Vercel env vars (`GEMINI_API_KEY`, `GROQ_API_KEY`), never in client/git.
